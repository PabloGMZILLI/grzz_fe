...
export default function BillboardDetails({ route, navigation }) {
    ...
    if (administrative)
        useLayoutEffect(() => {
            UserService.getUserQuestions(userDisplayed.id, user.id).then(
                (res) => setUserQuestions(res)
            );
        }, []);
        ...
        if (element) {
            return (
                <View key={index} style={styles.question}>
                    ...
                    <Text style={{ fontSize: 15 }}>
                        <Text style={{ fontWeight: "bold" }}>Questão: </Text>{" "}
                        {element.question_label}
                    </Text>
                    <Text style={{ fontSize: 15 }}>
                        <Text style={{ fontWeight: "bold" }}>
                            Resposta selecionada:{" "}
                        </Text>{" "}
                        {element.answer_checked_label}
                    </Text>
                    <Text style={{ fontSize: 15 }}>
                        <Text style={{ fontWeight: "bold" }}>
                            Resposta correta:{" "}
                        </Text>
                        {element.correct_answer_label}
                    </Text>
                    <Text style={{ fontSize: 15 }}>
                        <Text style={{ fontWeight: "bold" }}>
                            Pontos ganhos:{" "}
                    ...
                    <Text style={{ fontSize: 15 }}>
                        ...
                        {element.timespent > 60
                            ? element.timespent / 60 + " min/s"
                            : element.timespent + " segundos"}{" "}
                    </Text>
                </View>
            );
        }
    ...
    function nameNormalized(name, lastname) {
        ...
        let fullname =
            firstname + " " + lastname[0].toUpperCase() + lastname.substr(1);
        return fullname;
    }
                        ...
                        <View style={{ flexDirection: "row" }}>
                            ...
                            <View style={{ marginLeft: 20 }}>
                                ...
                                <Text h7>
                                    ...
                                    {userDisplayed && userDisplayed.points
                                        ? userDisplayed.points.toFixed(2)
                                        : null}
                                </Text>
                                <Text h7>
                                    <Text style={{ fontWeight: "bold" }}>
                                    ...
                                    {userDisplayed && userDisplayed.workspace}
                                </Text>
                            </View>
                        </View>
                    </View>
                    ...
                    {administrative ? (
                        <View style={mainStyle.container}>
                            ...
                            <Text h7>
                                ...
                                peguntas respondidas{" "}
                            </Text>
                            <TouchableOpacity
                                style={mainStyle.redButton}
                                onPress={() => toggleQuestions(!showQuestions)}
                            >
