...
const QuizDetails = ({route}) => {
    ...
    async function saveAnswer(quizId, questionId, answerId, timespent = 0) {
    ...
    quiz.questions.map(
        question => question.answers.map(
            answer => {
                if (answer.checked){
                    saveAnswer(quiz.id, question.id, answer.id, question.timespent);
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.textContainer}>
                ...
                <Text style={ styles.resultsTexts }>
                    Você acertou: {correctQuestions.length}{ correctQuestions.length > 1 || correctQuestions.length < 1 ? ' questões.' : ' questão.' }
                </Text>
                <Text style={ styles.resultsTexts }>Sua pontuação foi de: {points} pontos</Text>
            </View>
